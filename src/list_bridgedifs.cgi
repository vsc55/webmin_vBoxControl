...
my (@TABHEAD) = ($text{'tabhead_bridge'},$text{'tabhead_bridgedesc'});

print &ui_columns_start(\@TABHEAD, 100, 0, \@TD);
foreach my $DUMMY (@BRIDGEIF)
	{
	print ui_columns_row(["<b> Bridge Adapter ${$DUMMY}{'Name'}</b>"],["colspan=2 align=center"]);
...
if ($in{'print'})
	...
	{
	print "<br>";
	print "<a href='#' onclick=\"javascript:window.open('list_bridgedifs.cgi?print=1','popup', 'toolbar=0,width=840, height=600')\">";
